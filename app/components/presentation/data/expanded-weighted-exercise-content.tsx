...
import {
  formatWeeklyRate,
  getUsualRepRange,
  getWeightedExerciseSetsPerWeek,
} from '@/utils/weighted-exercise-stats';
...
export function ExpandedWeightedExerciseContent(props: {
  emptyRoute: '/stats' | '/(tabs)/progress';
}) {
...
function OverallStatsGrid({ stats }: { stats: WeightedExerciseStatistics }) {
  const { t } = useTranslate();
  const timePeriod = useAppSelector((x) => x.stats.overallViewTime);
  const setsPerWeek = getWeightedExerciseSetsPerWeek(stats, timePeriod);
